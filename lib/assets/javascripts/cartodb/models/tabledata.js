cdb.admin.CartoDBTableData = cdb.ui.common.TableData.extend({
  ...
  */
  ...
  discreteHistogram: function(nbuckets, column, callback, error) {
    ...
    var queryOverMax = 'SELECT count(*) as value FROM (select DISTINCT <%= column %> from <%= table %>) as tmp LIMIT 5000';
    ...
    this._sqlQuery(sql, function(data) {
      ...
      if (count > nbuckets) {
        reached_limit = true;
        var sql = _.template(queryOverMax, {
          column: column,
          table: this.table.id
        });
        this._sqlQuery(sql, function(data) {
          callback({ rows: [], reached_limit: reached_limit, total_count: data.rows[0].value });
        }.bind(this));
      } else {
        callback({ rows: data.rows, reached_limit: reached_limit, total_count: count });
      }
    }.bind(this));
  },
    ...
    this._sqlQuery(_.template('select * from (<%= sql %>) __wrapped limit 0')({ sql: sql }), function(data) {
      ...
      var process = function (self) {
        if (tmpSchemaAlias === undefined) {
          self.table.fetch(null, function () {
            this.load_count++;
            if (this.load_count < 1) {
              tmpSchemaAlias = self.table.get('column_aliases');
              process(self);
            } else {
              tmpSchemaAlias = {};
              process(self);
        ...
        this.alias_query_schema = this._aliasSchemaFromQueryFields(data.fields);
        if (!this.table.isInSQLView()) {
          if ('the_geom' in this.query_schema) {
            delete this.query_schema['the_geom_webmercator'];
          }
        }
        if (this.table.get('name_alias')) {
          this.table.trigger('update_layer_alias', this.table.get('name_alias'));
        }
        cdb.ui.common.TableData.prototype.fetch.call(this, opts);
      }.bind(this);

      var tmpSchemaAlias = self.table.get('column_aliases');
      if (tmpSchemaAlias !== undefined) {
        process(self);
      } else {
        self.table.fetch(null, function () {
