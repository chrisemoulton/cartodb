...
cdb.admin.Visualization = cdb.core.Model.extend({
   ...
   */
  tableMetadata: function(skipUserLayers) {
    if (!this._metadata) {
      if (this.get('table')) {
        this._metadata = new cdb.admin.CartoDBTableMetadata(this.get('table'));
      } else {
        this._metadata = new cdb.admin.CartoDBTableMetadata({id: this.get('table_id'), name: this.get('name'), skip_user_layers: skipUserLayers});
      }
    }
   ...
   */
  viewUrl: function(currentUser, skipUserLayers) {
    ...
    if (this.isVisualization() || _.isUndefined(this.get('type'))) {
      ...
      return new cdb.common.DatasetUrl({
        base_url: userUrl.urlToPath('tables', this.tableMetadata(skipUserLayers).getUnquotedName())
      });
    }
   },
...
cdb.admin.Visualizations = Backbone.Collection.extend({
  ...
  url: function(method) {
    ...
    if (opts.get('deepInsights')) {
      u += '/api/v1/bivisualizations';
      u += '?page=' + opts.get('page') + '&per_page=' + opts.get("per_page");
    } else {
      var version = cdb.config.urlVersion('visualizations', method);
      var useIndex = opts.get('use_index_api') ||
                    (opts.has('q') && opts.get('q') !== '') ||
                    (opts.has('name') && opts.get('name') !== '') ||
                     opts.get('samples');
      u += '/api/' + version + (useIndex ? '/viz/' : '/viz/list/');
