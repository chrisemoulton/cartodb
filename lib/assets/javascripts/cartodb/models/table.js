...
(function() {
  ...
  cdb.admin.CartoDBTableMetadata = cdb.ui.common.TableProperties.extend({
    ...
    initialize: function() {
      ...
      this._prepareSchema();
      var turnOffGetLayersAttr = user_data.get_layers === true && this.get('skip_user_layers');
      if (turnOffGetLayersAttr) {
        user_data.get_layers = false;
      }
      this.user = new cdb.admin.User(user_data);
      if (turnOffGetLayersAttr) {
        user_data.get_layers = true;
      }
    ...
    aliasNames: function (sc) {
      sc = sc || 'column_aliases';
      var column_aliases = this.get(sc) ? this.get(sc) : {};

      return column_aliases;
    },

    getColumnAlias: function (columnName) {
      for (var key in this.attributes.column_aliases) {
        if (key === columnName) {
          return this.attributes.column_aliases[key];
        }
      }
    ...
    updateAliasKey: function(newName, oldName) {
      var column_aliases = this.get('column_aliases');
      if(column_aliases[oldName]) {
        column_aliases[newName] = column_aliases[oldName];
        delete column_aliases[oldName];
        this.set('column_aliases', column_aliases);
        this.save(null, {
          error: function (e, resp) {
            cdb.log.error("can't update alias column key");
            self.error('error updating alias column', resp);
    ...
    renameColumn: function(columnName, newName, opts) {
      ...
      c.save(null,  {
          success: function(mdl, data) {
            ...
            self.updateAliasKey(newName, oldName);
            self.data().fetch();
    ...
    renameAlias: function (columnName, aliasOldName, newAliasName, opts) {
      ...
      this.set('column_aliases', aliasList);
    ...
    fetch: function (opts, cb) {
      ...
      $.when(xhr).done(function() {
        ...
        // Need delay so table model has time to update before calling cb
        setTimeout(function () {
          if (cb) {
            cb();
          }
    ...
    /**
     * Mark this table as having been displayed in a georeference
     * dialog, in which case it should no longer be automatically
     * shown in a georeference dialog.
     * @method markGeoreferenceDialogShown
     */
    markGeoreferenceDialogShown: function() {
      localStorage[this._dialogShownKey()] = true;
    },

    /**
     * Indicate whether a georeference dialog has been shown for this
     * table, in which case it should no longer be automatically
     * shown in a georeference dialog.
     * @method georeferenceDialogShown
     */
    georeferenceDialogShown: function() {
      return !!localStorage[this._dialogShownKey()];
    },

    /**
     * Key for dialog shown flag in local storage.
     * @method _dialogShownKey
     * @private
     */
    _dialogShownKey: function() {
      return 'georefNoContentWarningShowed_' + this.id + '_' + this.get('map_id');
    },
