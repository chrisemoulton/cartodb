...
cdb.admin.models.Filter = cdb.core.Model.extend({
  ...
  getLowerLimit: function(){
    return this.get('lower_limit');
  },

  getUpperLimit: function(){
    return this.get('upper_limit');
  },
...
cdb.admin.models.FilterDiscrete = cdb.core.Model.extend({
  ...
  _updateHist: function(hist) {
    if (hist.reached_limit) {
       this.set("reached_limit", hist.reached_limit);
       (hist.total_count) ? this.set("total_count", hist.total_count) : null;
       this.items.reset(this.get('items'));
    } else {
  ...
  getSQLConditionForString: function() {
    ...
    if (items.length > 0) {
      ...
      if (items.length == this.items.length && !this.get('reached_limit')) {
  ...
  toJSON: function() {
    return {
      ...
      list_view:     true,
      column_type:   this.get('column_type'),
      total_count:   this.get('total_count') || null
    }
  }
});
