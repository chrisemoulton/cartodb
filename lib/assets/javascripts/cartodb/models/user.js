...
cdb.admin.User = cdb.core.Model.extend({
  ...
  initialize: function(attrs) {
    ...
    this.profileAttributes = new cdb.admin.ProfileAttributes(
  ...
  canAddLayerTo: function(map) {
    ...
    if (!this.featureEnabled("bbg_disabled_shared_empty_dataset")) {
      var emptyDatasetName = cdb.config.get('shared_empty_dataset_name');

      for (i = 0; i < numDataLayers; ++i) {
        if (dataLayers[i].get('table_name') === emptyDatasetName) {
          numVisibleLayers--;
          break;
        }
      }
    ...
    return numVisibleLayers < this.getMaxLayers();
  },
