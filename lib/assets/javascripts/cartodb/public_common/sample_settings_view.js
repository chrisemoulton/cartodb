...
module.exports = cdb.core.View.extend({
  ...
  _sampleMapSave: function() {
    if(this.currentUser.featureEnabled('bbg_rtr_disable_sample_save_as')) {
      this._saveMap();
    }
    else {
      this._saveAsMap();
  ...
  _copyLink: function() {
    // Display Bloomberg terminal link
    // Link is of form: {MAPS SAMPLE:<ID>}
    var bbg_link = 'MAPS SAMPLE:' + this.vizId;

    var link = this.currentUser.viewUrl() + "/api/v1/sample_link/" + this.vizId;

    var CopyLinkDialog = BaseDialog.extend({
      render_content: function() {
        return cdb.templates.getTemplate('common/views/samples_copy_link')({
          link: link,
          bbg_link: bbg_link
        });
      }
    ...
    var dialog = new CopyLinkDialog();

    // To render & show initially (only to be called once):
    dialog.appendToBody();

    // Copy to clipboard
    var clipboardLink = document.querySelector('.js-bbg-sample-link');
    var range = document.createRange();
    range.selectNode(clipboardLink);
    window.getSelection().addRange(range);
    try {
      var successful = document.execCommand('copy');
      if(!successful) {
        console.log('Copy bbg link command failed');
      }
    } catch(err) {
