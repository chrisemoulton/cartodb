        ...
        this._exportMapModel.bind('change:state', function () {
          ...
          if (state === 'complete') {
            ...
            this._importModel = new ImportModel({
              endPollCallback: function( state ) {
                console.log('endPoll callback state: ' + state );
                if(state === 'complete') {
                  loadingView.close();
                }
                else {
                  console.log('end poll import state: ' + state);
                  return cdb.templates.getTemplate('common/templates/fail')({
                    msg: 'Import has failed'
                  });
                }
              }
            });
            console.log(this._importModel)

            this._importModel.bind('change:state', function() {
              var importState = this._importModel.get('state');
              console.log('Import state: ' + state)
              if(state === 'complete') {
                console.log(this._importModel.get('item_queue_id'));
              }
            ...
            this._importModel.createImport({
              type: 'url',
              create_vis: true,
              value: encodeURI(newBaseUrl + url)
            });
