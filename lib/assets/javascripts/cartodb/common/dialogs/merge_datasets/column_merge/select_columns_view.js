...
module.exports = cdb.core.View.extend({
  ...
  render: function() {
    var leftKeyColumnName;
    var leftKeyColumnAlias;
    var leftKeyColumnType;
    
    var rightKeyColumnName;
    var rightKeyColumnAlias;
    var rightKeyColumnType;

    if (this.model.get('leftKeyColumn')) {
      leftKeyColumnName = this.model.get('leftKeyColumn').get('name');
      leftKeyColumnAlias = this.model.get('leftKeyColumn').get('alias');
      leftKeyColumnType = this.model.get('leftKeyColumn').get('type');
    }

    if (this.model.get('rightKeyColumn')) {
      rightKeyColumnName = this.model.get('rightKeyColumn').get('name');
      rightKeyColumnAlias = this.model.get('rightKeyColumn').get('alias');
      rightKeyColumnType = this.model.get('rightKeyColumn').get('type');
    }
    var $el = $(
    ...
    var rightTableName = '';
      if (this.options.user.featureEnabled('aliases')) {
        if (this.model.get('rightTableData').name_alias) {
          rightTableName = this.model.get('rightTableData').name_alias + '(' + this.model.get('rightTableData').name + ')';
        } else {
          rightTableName =  this.model.get('rightTableData').name;
        }
      } else {
        if (this.model.get('rightTableData').name_alias) {
          rightTableName = this.model.get('rightTableData').name_alias;
        } else {
          rightTableName =  this.model.get('rightTableData').name;
        }
      }
