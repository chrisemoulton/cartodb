...
module.exports = cdb.core.Model.extend({
  ...
  _initGeocodeStuff: function() {
    // Determine if toggle override/forceAllRows should be enabled
    var toggleForceAllRowsEnabled =
      this.get('user').featureEnabled('georeference_toggle_force_all_rows_enabled');
    // Create model, setting forceAllRows to true if toggling is
    // not enabled
    var geocodeStuff = new GeocodeStuffModel({
      tableName: this.get('table').get('id'),
      toggleForceAllRowsEnabled,
      forceAllRows: !toggleForceAllRowsEnabled
    });
    this.set('geocodeStuff', geocodeStuff);
  },
  ...
  _initOptions: function() {
    ...
    var enabledForAdvancedGeoreference =
      this.get('user').featureEnabled("georeference_advanced_features");

    var models = !enabledForAdvancedGeoreference
               ? defaultModels
               : defaultModels.concat([
                   new CityNamesModel({
                     geocodeStuff: geocodeStuff,
                     columnsNames: columnsNames,
                     columns: columns
    ...
    this.set('options', new Backbone.Collection(models));
  ...
  _maybeDisabledStreetAddresses: function() {
    ...
    if (this._isGoogleMapsEnabled()) {
      if (!isGmeGeocoderEnabled) {
        this._disableTab('Google Maps geocoder is not configured. Please contact us at sales@cartodb.com', this._streetAddrTabModel());
      }
    } else {
      if (isGmeGeocoderEnabled) {
        this._disableTab('Geocoder is not configured properly. Please contact us at sales@cartodb.com', this._streetAddrTabModel());
      } else if (!this._userGeocoding().hasQuota()) {
        // Credits are only used for users that use non-GME geocoder
        this._disableTab('Your geocoding quota is not defined. Please contact us at sales@cartodb.com', this._streetAddrTabModel());
      }
    }
  },
