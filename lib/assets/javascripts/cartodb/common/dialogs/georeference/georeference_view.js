...
module.exports = BaseDialog.extend({
  ...
  defaults: {
    // If the table has no georeferenced data, show a warning
    showNoGeoreferenceWarning: false,
  },
  ...
  render_content: function() {
    ...
    var showNoGeoreferenceWarning =
      !table.isGeoreferenced() &&
      table.data().length > 0 &&
      this.options.showNoGeoreferenceWarning;
    var $el = $(
      this.getTemplate('common/dialogs/georeference/georeference')({
        showNoGeoreferenceWarning
      })
    );
  ...
  /**
   * Signal that view is closed - called either in upon
   * selecting a geocoding option or when view is closed
   * without geocoding.
   */
  _cancel: function() {
    this.trigger('closed');
    BaseDialog.prototype._cancel.apply(this, arguments);
  },
