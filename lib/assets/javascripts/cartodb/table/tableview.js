...
(function() {
    ...
    cdb.admin.TableView = cdb.ui.common.Table.extend({
      ...
      /**
       * Determine if table should be georeferenced, meaning
       * it has either not been georeferenced or a dialog for
       * georeferencing the table has already been shown.
       * @method _checkGeoRef
       * @private
       */
      _checkGeoRef: function() {
        var self = this;
        var table = this.model;

        function tableLoadedCallback() {
          if (!table.isGeoreferenced()
          &&  !table.isSync()
          &&  !table.georeferenceDialogShown()) {
            self._launchGeoreferenceDialog();
          }
        }

        if(!table.data().fetched) {
          table.bind('dataLoaded', function bindCallback() {
            table.unbind('dataLoaded', bindCallback);
            tableLoadedCallback();
          });
        } else {
          tableLoadedCallback();
      ...
      headerView: function(column) {
        ...
        if(column[1] !== 'header') {
          ...
          .bind('georeference', this._launchGeoreferenceDialog, this)
      ...
      /**
       * Launch georeferencing dialog, provided table is enabled for georeferencing.
       * @method _launchGeoreferenceDialog
       * @private
       */
      _launchGeoreferenceDialog: function() {
          var dlg;
          ...
          if (!this.options.geocoder.isGeocoding() && !tableIsReadOnly && canAddGeocoding) {
            if (this.georefDialog) {
              this.georefDialog.remove();
            }
            dlg = new cdb.editor.GeoreferenceView({
              table:  this.model,
              user:   this.user,
            });
            dlg.bind('closed', function () {
              var table = this.model;
              table.markGeoreferenceDialogShown();
            }, this);
            this.georefDialog = dlg;
      ...
      */
      ...
      /**
       * Perform view cleanup as needed.
       */
      clean: function() {
        if (this.georefDialog) {
          this.georefDialog.remove();
