  ...
  **/
  ...
  cdb.admin.mod.Filter = cdb.core.View.extend({
    ...
    events: {
      ...
      'click  .date-min'            :  '_showCalendarMin',
      'click  .date-max'            :  '_showCalendarMax',
      'focus  .date-max'            :  '_showCalendarMax',
      ...
      'click  .min-plus'            :  '_adjustMinField',
      'click  .max-plus'            :  '_adjustMaxField',
      'click  .min-minus'           :  '_adjustMinField',
      'click  .max-minus'           :  '_adjustMaxField'
    },
    ...
    _adjustMinField: function (e) {
      e.preventDefault();
      e.stopPropagation();
      this.$el.find('.min').val( function(i, oldval) {
        var newVal = parseFloat(oldval);
        if ($(e.currentTarget).hasClass('min-plus')) {
          newVal++;
          if (newVal > this.model.get('upper_limit')){
            newVal = parseFloat(oldval);
          }
        } else {
          newVal--;
          if (newVal < this.model.get('lower_limit')){
            newVal = parseFloat(oldval);
          }
        }
        this.model.set('lower', newVal);
        return newVal;
      }.bind(this));
    },

    _adjustMaxField: function (e) {
      e.preventDefault();
      e.stopPropagation();
      this.$el.find('.max').val( function(i, oldval) {
        var newVal = parseFloat(oldval);
        if ($(e.currentTarget).hasClass('max-plus')) {
          newVal++;
          if (newVal > this.model.get('upper_limit')){
            newVal = parseFloat(oldval);
          }
    ...
    _setupCalendar: function() {
      ...
      this.calendar = this.$el.find(".ui-datepicker-div").datepicker({
        changeMonth: true,
        changeYear: true,
        rangeSelect: true,
        numberOfMonths: 1,
    ...
    _showCalendarMax: function(e){
      if (e) {
        e.preventDefault();
        e.stopPropagation();
      }
      this.calInputSelect = 'max';
      this.calendar.datepicker('setDate', new Date(this.$el.find('.date-max').val()));
      if (this.calOpen) {
        this.calendar.show();
    ...
    _showRangeInput: function(e){
      e.preventDefault();
      e.stopPropagation();
      if (this.model.get('lower_limit') === this.model.get('upper_limit')) {
        return;
      }
      ...
      setTimeout(function() {
        ...
        if (hist) {
          if (hist.length > 1) {
            ...
            self.$(".empty").hide();
            if (self.isDate) {
              self.$('.range-text').text(self._formatDate(new Date(self.model.get('lower_limit')),new Date(self.model.get('lower_limit'))).min);
            } else {
              self.$('.range-text').text(self.model.get('lower_limit'))
            }
            self.$(".range-date-text").hide();
          }
        } else {
    ...
    _renderRangeText: function (upper, lower, minMax) {
      ...
      if (!_.isNaN(upper) && !_.isNaN(lower)) {
        if (this.isNumber) {
          var minVal = this.abbreviateNumber(minMax.min);
          var maxVal = this.abbreviateNumber(minMax.max);
          ...
          this.calendar.datepicker( "option", "maxDate", new Date(this.model.getUpperLimit()));
          this.calendar.datepicker( "option", "minDate", new Date(this.model.getLowerLimit()));
        }
      }
    },
    ...
    abbreviateNumber: function (value) {
      value = Math.round(value);                              //remove all decimal places
      var newValue = value;                                   //set newValue as value for return if number is less then 1000 or null or NaN
      if (value >= 1000) {                                    //dont touch numbers under 1000
          var suffixes = ["", "k", "M", "B","T", "P", "E"];   //List of suffixes to user
          var suffixNum = Math.floor( (""+value).length/3 );  //Determine which suffix to use
          var shortValue = '';                                //shortValue place holder
          for (var precision = 2; precision >= 1; precision--) {  //for loop to reduce number to proper abbreviation
              shortValue = parseFloat( (suffixNum != 0 ? (value / Math.pow(1000,suffixNum) ) : value).toPrecision(precision)); //reduce number basedon prefix and precision for loop value
              var dotLessShortValue = (shortValue + '').replace(/[^a-zA-Z 0-9]+/g,''); //remove decimal to test length
              if (dotLessShortValue.length <= 2) { break; } //test length of number if reduced to two values then stop otherwise continue
          }
          if (shortValue % 1 != 0)  shortNum = shortValue.toFixed(1); //if shortvalue is whole number let it be else use precision 1
          newValue = shortValue+suffixes[suffixNum];                  //add suffix to value
      }
      return newValue;
    },
    ...
    _calculateTicksNum: function(x){
      ...
      return axis.tickValues(ticks).tickFormat(function(d, idx) {
        ...
        switch (idx) {
          case 0:
            return this.abbreviateNumber(this.model.getLowerLimit());
          case 1:
            return this.abbreviateNumber(bucketSize + this.model.getLowerLimit());
          case 2:
            return this.abbreviateNumber(bucketSize * 2 + this.model.getLowerLimit());
          case 3:
            return this.abbreviateNumber(bucketSize * 3 + this.model.getLowerLimit());
          case 4:
            return this.abbreviateNumber(this.model.getUpperLimit());
          }
      }.bind(this));
    },
    ...
    _barChart: function() {
      ...
      function chart(div) {
        ...
        if (g.empty()) {
          ...
          if (self.model.get('lower_limit') !== self.model.get('upper_limit')) {
