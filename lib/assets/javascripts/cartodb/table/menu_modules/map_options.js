...
cdb.admin.MapOptionsDropdown = cdb.core.View.extend({
  ...
  initialize: function () {
    ...
    var profAttrs = new cdb.admin.ProfileAttributes(profile_attributes);
    this.enableLockPan = profAttrs.getAttribute("enableLockPan");
    this.enableLockZoom = profAttrs.getAttribute("enableLockZoom");
  ...
  _setupOptions: function () {
    ...
    // Sync map options with relevant flags stored by map model
    var mapLevelFlagNames = ['scrollwheel', 'legends', 'lock_pan', 'lock_zoom'];
    var mapLevelFlags = _.object(_.map(mapLevelFlagNames, function (flagName) {
      return [flagName, !!this.options.vis.map.get(flagName)];
    }, this));

    this.mapOptions.set(mapLevelFlags);
  ...
  render: function () {
    ...
    if (this.enableLockPan) {
      switches.push('lock_pan')
    }
    if (this.enableLockZoom) {
      switches.push('lock_zoom')
    }
