...
cdb.admin.MetadataForm = cdb.core.View.extend({
  ...
  initialize: function () {
    ...
    this._categoryTree = new cdb.admin.CategoryTree({
      user: this.options.user,
      rootId: this.model.isDataset() ? this._DATASETS_CATEGORY : this._SAMPLE_MAPS_CATEGORY
    });
    this.isSampleMapsUser = this.user.get('username') == cdb.config.get('sample_maps_username');
  ...
  render: function () {
    ...
    this.$el.html(
      this.template({
        ...
        isSampleMapsUser: this.isSampleMapsUser
      })
    );
    this._initViews();
    return this;
  ...
  _initCategoryCombos: function() {
    var self = this;
    var categoryItem = this._categoryTree.getCategory(this.model.get('category'));
    var isParentCategory = categoryItem ? (categoryItem.parent_id == this._DATASETS_CATEGORY || categoryItem.parent_id == this._SAMPLE_MAPS_CATEGORY) : false;
    this.model.set('parent_category', categoryItem ? (isParentCategory ? categoryItem.id : categoryItem.parent_id) : null);
    this._categoryDropdown = new cdb.forms.Combo({
      ...
      model: this.model,
      disabled: !this.model.isMetadataEditable(),
      extra: this._getCategoriesForFormsCombo(this.model.isDataset() ? this._DATASETS_CATEGORY : this._SAMPLE_MAPS_CATEGORY)
    ...
    this._subCategoryDropdown = new cdb.forms.Combo({
      ...
      property: 'category',
      model: this.model,
      disabled: isParentCategory || !this.model.isMetadataEditable(),
      extra: this._getCategoriesForFormsCombo(categoryItem ? (isParentCategory ? categoryItem.id : categoryItem.parent_id): null, 'Select a sub-category...')
  ...
  _onCategorySelectionChanged: function(ev) {
    ...
    var subCategoryOptions = this._getCategoriesForFormsCombo(parentCategoryId, 'Select a sub-category...');
    ...
    this._subCategoryDropdown = new cdb.forms.Combo({
      ...
      property: 'category',
      model: this.model,
      disabled: subCategoryOptions.length <= 1 || !this.model.isMetadataEditable(),
      extra: subCategoryOptions
    }).bind("change", self._onSubcategorySelectionChanged, self);
    this.addView(this._subCategoryDropdown);
    this.$('.subcategory-list').append(this._subCategoryDropdown.render().el);

    if (subCategoryOptions.length <= 1) {
      this.model.set('category', parentCategoryId);
      this._onSubmit(ev);
    }
  },
  ...
  _getCategoriesForFormsCombo: function(parentId, placeholderTitle) {
    ...
    var items = [[placeholderTitle || 'Select a category...', null]];
