  ...
  _addBasemapDropdown: function() {
    ...
    if (!this.basemapDropdown) {
      ...
      if (this.vis.get("type") !== "table") {
        ...
        <a href="#" class="option-button dropdown chk_global_winds live" title="Click to toggle Wind Layer"><div class="toggle"></div>Live Winds</a>\
        <a href="#" class="option-button dropdown chk_global_winds hurr" title="Click to toggle Wind Layer"><div class="toggle"></div>Hurr Winds</a>\
        ');

        $(".map-options").append($options);

        var self = this;        
        $('.chk_global_winds.live').click(function(e){
          var toggle = $(e.target).find('.toggle');
          self.mapView.hideWindLayer();
          if (toggle.hasClass('selected')) {
            toggle.removeClass('selected');
          } else {
            self.mapView.showWindLayer();
            toggle.addClass('selected');
            $('.chk_global_winds.hurr .toggle').removeClass('selected');
          }
        });

        $('.chk_global_winds.hurr').click(function(e){
          var toggle = $(e.target).find('.toggle');
          self.mapView.hideWindLayer();
          if (toggle.hasClass('selected')) {
            toggle.removeClass('selected');
          } else {
            self.mapView.showWindLayer(new Date("September 19, 2017 11:00:00"));
            toggle.addClass('selected');
            $('.chk_global_winds.live .toggle').removeClass('selected');
          }
        });
