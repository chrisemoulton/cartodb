...
if (user_data && user_data.feature_flags && user_data.feature_flags.indexOf('bbg_new_search') != -1) {
  SearchInput = Backbone.View.extend({
    ...
    invalidateSearchIndex: function() {
    },
  ...
  SearchInput = Backbone.View.extend({
    ...
    _updateSearchIndex: function() {
      var searchItems = [];
      var id = 0;
      var i, numItems = this._allItems.length, item, name;
      for (i = 0; i < numItems; ++i) {
          item = this._allItems[i];
          searchItems.push({ id: i, name: item.displayName, tags: item.tags });
      }

      this._fuse = new Fuse(searchItems, { keys: ["name", "tags"], threshold: 0.4 });
    },

    _loadDatasetList: function() {
      var self = this;
      
      $.ajax({
          url: "../../api/v1/viz/list/?types=table,remote"
      })
      .done(function(response) {
          self._allItems = [];
          ...
          for (i = 0; i < numResults; ++i) {
            result = results[i];
            self._allItems.push({ id: result.id, type: result.type, name: result.name,
              displayName: result.name_alias || result.display_name || result.name,
              tags: result.tags ? result.tags.join(', ') : '', needs_cd_import: result.needs_cd_import });
          }
          self._updateSearchIndex();
      })
      .fail(function() {
    ...
    _doSearch: function() {
      ...
      var fuseResults = self._fuse.search(self.query);
      var filtered = [];
      _.each(fuseResults, function(item, index) {
        if (filtered.length >= self._numMaxResults) {
          return;
        }
        filtered.push(self._allItems[fuseResults[index].id]);
      });

      self._setItems(filtered);
    },
    ...
    _renderListItem: function(index, item, selected) {
      ...
      var name, tags = item.tags;
      var startPos = item.displayName.toLowerCase().indexOf(query);
      if (startPos == -1) {
          name = item.displayName;
      } else {
          name = item.displayName.substr(0, startPos) + '<span class="found-sub">' + item.displayName.substr(startPos, queryLen) + '</span>' + item.displayName.substr(startPos + queryLen);
      }
      
      startPos = tags != null ? tags.toLowerCase().indexOf(query) : -1;
      if (startPos != -1) {
    ...
    _onItemClicked: function(item) {
      ...
      if (item.type === 'remote') {
        var d = {
          ...
          needs_cd_import: item.needs_cd_import,
          hide_progress_display: !this.user.featureEnabled('bbg_show_import_progress'),
          size: undefined//mdl.get('external_source') ? mdl.get('external_source').size : undefined
        };
        table.backgroundPollingModel.bind('importCompleted', function importCompletedCallback() {
          table.backgroundPollingModel.unbind('importCompleted', importCompletedCallback);
          table.globalError.hide('dataset-load');
          table.menu.trigger('show-panel');
        }, self);
        table.backgroundPollingModel.bind('importFailed', function importFailedCallback() {
          table.backgroundPollingModel.unbind('importFailed', importFailedCallback);
          table.globalError.hide('dataset-load');
          table.globalError.showError('There was an error loading the dataset.', 'error', 10000);
        }, self);
        table.globalError.showError('Loading', 'load', 0, 'dataset-load');
        cdb.god.trigger('importByUploadData', d, self);
      } else {
        table.globalError.showError('Loading', 'load', 0, 'dataset-load');
    ...
    _onKeyUp: function(e) {
      ...
      var query = this.$el.val().toLowerCase();
      if (query != this.query) {
        this.query = query;
...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    this._chooseBaseLayer();
    this._addBindings();

    if (!this.user.profileAttributes.get('enableAVMMExport')
    &&  !this.user.featureEnabled("bbg_pro_ui")) {
  ...
  _addSearchBar: function() {
    ...
    this.map.layers.bind('add', function() {
      this._searchInput.invalidateSearchIndex();
    }, this);
  },
  ...
  /**
   * Select the appropriate base layer for the map.
   * If a base layer for the map is set, enforce that
   * it is one of the allowed base layers. If no base
   * layer is set, do nothing.
   * @private
   */
  _chooseBaseLayer: function() {

    var map = this.map;
    var baseLayer = map.getBaseLayer();
    // Ensure base layer is defined and that base layer is not a data layer
    if (baseLayer && baseLayer.get('type') != "Plain") {

      // Enforce that the maps's saved base layer is one
      // of the base layers for which the user is enabled
      var baseLayers = this.options.baseLayers;
      var enabledForSavedBaseLayer =
        baseLayers.filter(bl => baseLayer.isEqual(bl)).length > 0;

      if (!enabledForSavedBaseLayer) {
        // Base layer must be changed through the changeProvider interface.
        // If the same provider is passed, only the baselayer is changed
        var provider = map.get('provider');
        var defaultBaseLayer = baseLayers.at(0);
        map.changeProvider(provider, defaultBaseLayer);
      }
      ...
      if (!this.georeferenced) {
        if (this.table.data().length > 0) {
          if (this.table.isSync()) {
            this._showNoGeoWarning();
          }
          else {
            // Show georeference dialog without a warning message
            this.showNoGeoRefWarning(dialogOnly = true);
          }
        }
      }
    }
   ...
   * @param dialogOnly Show georeference dialog but hide warning message
   */
  showNoGeoRefWarning: function(dialogOnly = false) {
    // if the dialog has already been shown for this table, we don't show it again
    if(!this.table.isInSQLView() && !this.table.georeferenceDialogShown()) {

      if (this.noGeoRefDialog) {
        this.noGeoRefDialog.remove();
      }

      this.noGeoRefDialog = new cdb.editor.GeoreferenceView({
        table: this.table,
        user: this.user,
        showNoGeoreferenceWarning: !dialogOnly,
      });
      // Prevent further geo ref popups once this popup has been used
      this.noGeoRefDialog.bind('closed', function() {
        this.table.markGeoreferenceDialogShown();
      }, this);
