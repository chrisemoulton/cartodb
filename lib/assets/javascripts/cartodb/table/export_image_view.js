...
cdb.admin.ExportImageView = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    _.bindAll(this, "_onResizeWindow", "_onChangeDimensions", "_onImageCallback", "_onAdvancedImageCallback", "_keydown", "_onDrag", "_onResize", "_updateHelpers", "_onMouseWheel", "_onMouseDown");
    ...
    var deepCopy = true;
    var defaultsCopy = $.extend(deepCopy, {}, this.defaults);
    this.options = _.extend(defaultsCopy, this.options, {
      ...
      _.each(this.header, function(header) {
        ...
        if (header.attributes && header.attributes.options && header.attributes.options.extra) {
          ...
          if (extras.headerType == 'title') {
            ...
            self.descriptionOverlay = header;
    ...
    if(this.options.map.attributes.provider == "openlayers") {
      this.$el.on("wheel", this._onMouseWheel);
  ...
  _onMouseWheel: function(evt){
    var event = new WheelEvent("wheel", {
      "deltaX": evt.originalEvent.deltaX,
      "deltaY": evt.originalEvent.deltaY,
      "deltaMode": evt.originalEvent.deltaMode,
      "clientX": evt.originalEvent.clientX,
      "clientY": evt.originalEvent.clientY
    });

    this.options.mapView.getNativeMap().getViewport().dispatchEvent(event);
  },

  _onMouseDown: function(evt){
    var event = new MouseEvent("mousedown", {
      "screenX": evt.originalEvent.screenX,
      "screenY": evt.originalEvent.screenY,
      "clientX": evt.originalEvent.clientX,
      "clientY": evt.originalEvent.clientY,
  ...
  render: function() {
    ...
    var overlays = [{
      overlay: this.titleOverlay,
      overlayClass: '.ExportHelperTitle'
    }, {
      overlay: this.descriptionOverlay,
      overlayClass: '.ExportHelperDescription'
    }];

    _(overlays).each((o) => {
      if (o.overlay) {
        var overlayStyles = o.overlay.getStyles();

        var overlayOpacity = overlayStyles['box-opacity'];
        var boxPadding = overlayStyles['box-padding'];
        var fontFamilyName = overlayStyles['font-family-name'];

        var header = self.$(o.overlayClass);
        if (overlayOpacity !== undefined) {
          header.css('background', 'rgba(0, 0, 0, ' + overlayOpacity + ')');
        }
        if (boxPadding !== undefined) {
          header.css('padding', boxPadding);
        }
        if (fontFamilyName !== undefined) {
          header.css('font-family', fontFamilyName);
        }
    ...
    // tell the text boxes to set the width and height after initial render
    var self = this;
    setTimeout(function() {
      self.options.custom_size && self.trigger("width_height_changed", self.width, self.height);
    }, 200);
    return this;
  },

  getOffset: function () {
    return {left: this.model.get('x'), top: this.model.get('y')};
  },
  ...
  _setupCanvas: function() {
    ...
    this.$(".CanvasExport").on('mousedown', function(e) {
      e.stopPropagation();
    });
  },
  ...
  onChangeWidthHeight: function() {
    var width  = this.model.get("width");
    var height = this.model.get("height");
    this.trigger("width_height_changed", width, height);
  },
  ...
  _onResize: function(e, ui) {
    ...
    var lastWidth = this.model.get("width");
    var lastHeight = this.model.get("height");
  ...
  clean: function() {
    ...
    if(this.options.map.attributes.provider == "openlayers") {
      this.$el.off("wheel", this._onMouseWheel);
