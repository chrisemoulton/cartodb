...
cdb.admin.ExportImageView = cdb.core.View.extend({
  ...
  render: function() {
    ...
    // Preserve title overlay styles including opacity,
    // which is not stored along with other title styles
    if (this.titleOverlay) {
      var overlayStyles = this.titleOverlay.getStyles();

      var overlayOpacity = overlayStyles['box-opacity'];
      var boxPadding = overlayStyles['box-padding'];

      var header = this.$('.ExportHelper--header');
      ...
      this.$('.ExportHelperTitle').css(overlayStyles);
    }
