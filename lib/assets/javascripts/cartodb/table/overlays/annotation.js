cdb.admin.overlays.Annotation = cdb.geo.ui.Annotation.extend({
  ...
  initialize: function() {
    ...
    this._customizeFormData();
  },

  _customizeFormData: function() {
    var user = new cdb.admin.User(user_data);
    var extraFonts = user.profileAttributes.get('extraFonts');
    if (_.isArray(extraFonts)) {
      if (this.form_data[0] && this.form_data[0].form['font-family-name']) {
        var fonts = this.form_data[0].form['font-family-name'].extra;
