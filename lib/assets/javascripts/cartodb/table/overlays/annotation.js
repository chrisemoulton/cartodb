cdb.admin.overlays.Annotation = cdb.geo.ui.Annotation.extend({
  ...
  initialize: function() {
    ...
    this.form_data = this._customizeFormData(default_form_data);
  },

  _customizeFormData: function(defaultFormData) {

    var deepCopy = true;
    var formData = $.extend(deepCopy, {}, defaultFormData);

    var user = new cdb.admin.User(user_data);
    var fontFamilies = user.profileAttributes.get('fontFamilies');
    var defaultFontFamilyName = user.profileAttributes.get('defaultFontFamilyName');
    var fontDropdownWidth = user.profileAttributes.get('fontDropdownWidth');

    var fontForm = formData[0] && formData[0].form['font-family-name'];
    if (fontForm) {
      if (_.isArray(fontFamilies)) {
        // Overwrite existing font family options
        fontForm.extra = fontFamilies;
      }

      if (fontDropdownWidth) {
        // Allow additional space for larger font names
        fontForm.width = fontDropdownWidth;
      }

      if (defaultFontFamilyName) {
        fontForm.value = defaultFontFamilyName;
      }
    }
  ...
  _applyStyle: function() {
    ...
    this._applySelectedFont(fontFamily);
  ...
  _applySelectedFont: function(fontFamily) {
    // Remove all font family classes
    var fontFamilyClasses = cdb.admin.overlays.Annotation.FONT_FAMILY_CLASSES;
    var classes = _.values(fontFamilyClasses).join(" ");
    this.$el.removeClass(classes);

    // Add class associated with selected font family
    var fontFamilyClass = fontFamilyClasses[fontFamily] || "";
  ...
  FONT_FAMILY_CLASSES: {
    ...
    "Lato"                  : "lato",
    "Graduate"              : "graduate",
    "Gravitas One"          : "gravitas_one",
    "Old Standard TT"       : "old_standard_tt",
    "BWHaasGrotesk-55Fixed" : "BWHaasGrotesk-55Fixed",
    "BWHaasGrotesk-65Fixed" : "BWHaasGrotesk-65Fixed",
    "BWHaasHead-25XThin"    : "BWHaasHead-25XThin",
    "BWHaasHead-55Roman"    : "BWHaasHead-55Roman",
    "BWHaasHead-65Medium"   : "BWHaasHead-65Medium",
    "BWHaasHead-75Bold"     : "BWHaasHead-75Bold",
    "BWHaasHead-95Black"    : "BWHaasHead-95Black",
    "BWHaasText-55Roman"    : "BWHaasText-55Roman",
    "BWHaasText-65Medium"   : "BWHaasText-65Medium",
    "BWHaasText-75Bold"     : "BWHaasText-75Bold",
  },
});
