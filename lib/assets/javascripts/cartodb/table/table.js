  ...
  var Table = cdb.core.View.extend({
    ...
    */
    ...
    _initModels: function() {
      ...
      // Enforce set of whitelisted basemaps, if one is specified in profile attributes.
      var profAttrs = cdb.admin.ProfileAttributes.load(this.vis.user);
      var whitelistedBasemaps = profAttrs.get('whitelistedBasemaps');

      function filterBasemaps(basemaps, whitelistedBasemaps) {
        // Inner join whitelist with basemaps
        var commonBasemaps =
          _(whitelistedBasemaps).filter(b => basemaps.hasOwnProperty(b.basemapCategory));

        // _.pick for an array of keys
        var pickMany = (obj, keys) => _.prototype.pick.apply(_(obj), keys);

        return _.object(_.map(commonBasemaps, b => [
          // Index common basemaps by category
          b.basemapCategory,
          // Pick only whitelisted labels within each category
          pickMany(basemaps[b.basemapCategory], b.basemapLabels)
        ]));
      }

      var addlBasemaps = whitelistedBasemaps
                       ? filterBasemaps(this.options.basemaps, whitelistedBasemaps)
                       : this.options.basemaps;

      _(addlBasemaps).each(function(catBasemaps, cat) {
        _(catBasemaps).each(function(m) {
          var baseTypes = {
