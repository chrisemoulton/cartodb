...
module.exports = cdb.core.View.extend({
  ...
  _counts: { types: { all: undefined, liked: undefined, locked: undefined, imported: undefined }, sample_types: {all: undefined, liked: undefined}, categories: {} },
  ...
  _initBinds: function() {
    var contentType = this.routerModel.get('content_type');
    var contentTypeKnown = contentType === 'maps' || contentType === 'datasets';
    if (contentTypeKnown) {
      this._fetchCounts(this, function() {
        // refresh view to show new counts if category tree already shown
        if (this.$el.find(".LeftMenuContent").length > 0) {
  ...
  _loadCategoryTree: function() {
    ...
    var rootId = isMaps ? this._SAMPLE_MAPS_CATEGORY : this._DATASETS_CATEGORY;
  ...
  _initCategoryTree: function(keepCategories) {
    ...
    if (!keepCategories) {
      ...
      var rootId = isMaps ? self._SAMPLE_MAPS_CATEGORY : self._DATASETS_CATEGORY;
    ...
    this.$el.html(
      this.template({
        ...
        counts: this._counts,
        samplesParentCategory: self._SAMPLE_MAPS_CATEGORY
      })
    );
