...
module.exports = cdb.core.View.extend({
  ...
  _onRouterChange: function(m, changes) {
    ...
    this._backgroundPollingView.createVis = true;
  },
  ...
  _fetchCollection: function(m, changes) {
    ...
    var order = params.order || this.localStorage.get("dashboard.order") || 'name';
    var ascOrder = params.asc_order != false;

    if (params.content_type === "maps") {
      order = 'updated_at';
      ascOrder = false;
    }

    var types = '';
    if (params.content_type === "datasets") {
      types = params.imported ? 'table' : 'table,remote';
    } else {
      types = 'derived';
    }
  ...
  _initViews: function() {
    var backgroundPollingModel = new DashboardBackgroundPollingModel({
      showGeocodingDatasetURLButton: this.user.featureEnabled('bbg_pro_ui'),
