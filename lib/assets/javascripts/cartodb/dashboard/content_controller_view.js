...
window.ListView = require('./list_view');
...
var SideMenuView = require('./side_menu_view');
...
module.exports = cdb.core.View.extend({
  ...
  _initViews: function() {
    ...
    childContainer.append(noResultsView.render().el);
    ...
    childContainer.append(loadingLibrary.render().el);
    this.addView(loadingLibrary);

    this.controlledViews['error'] = errorView;
    childContainer.append(errorView.render().el);
  ...
  _onRouterChange: function(m, c) {
    var blocks = ['sideMenu', 'filters', 'main_loader'];
  ...
  _onDataFetched: function() {
    var activeViews = ['sideMenu', 'filters', 'content_footer'];
    ...
    if (this.collection.size() === 0) {
      if (!tag && !q && shared === "no" && locked !== 'true' && !liked) {

        if (this.router.model.get('content_type') === "maps") {
          // If there is no maps, let's show create first map message
          activeViews = ['sideMenu', 'filters', this.collection.total_entries == 0 ? 'list' : 'no_results'];
        } else if (library) {
  ...
  _onShowLoader: function() {
    this._hideBlocks();
    this._showBlocks(['sideMenu', 'filters', 'main_loader']);
  },
